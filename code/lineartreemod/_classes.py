...
from ._criterion import hamming, crossentropy,balanced_hamming,balanced_crossentropy
...
CRITERIA = {"mse": mse,
            ...
            "balanced_hamming":balanced_hamming}
...
def _parallel_binning_fit(split_feat, _self, X, y,
    ...
    for col, _bin in zip(split_feat, bins):
        ...
        for q in _bin:
            ...
            if n_left < _self._min_samples_leaf or n_right < _self._min_samples_leaf:
                ...
                if debug:print("leaf skip",n_left,n_right,"thr",_self._min_samples_leaf)
                continue
            ...
            # store if best
            if debug:
                print("total",total_loss,"vs",loss)
            if total_loss < loss:
                split_t = q
                split_col = col
                loss = total_loss
    ...
    if debug:
        print("debugend",loss, split_t, split_col, left_node, right_node)
    return loss, split_t, split_col, left_node, right_node
...
class _LinearTree(BaseEstimator):
    ...
    def _split(self, X, y, bins,
        ...
        all_results = Parallel(n_jobs=n_jobs, verbose=0,
            ...
            delayed(_parallel_binning_fit)(
                ...
                loss - min(0,self.min_impurity_decrease),
                False and (self.min_impurity_decrease<0)
            )
        ...
        saved_splits = split_t,split_col
        # select best results
        ...
        if loss - _losses[_id_best] > self.min_impurity_decrease:
            ...
            # if self.min_impurity_decrease<0:
            #     print(loss - _losses[_id_best],loss ,_losses[_id_best],self.min_impurity_decrease)
            split_t = None
            split_col = None
            left_node = (None, None, None, None, None)
            right_node = (None, None, None, None, None)
        # if len(X)>6:
        #     assert self.min_impurity_decrease>=0 or split_t is not None, (len(all_results),_id_best,loss ,"lenX",len(X),_losses,self.min_impurity_decrease,saved_splits)
    ...
    def _fit(self, X, y, sample_weight=None):
        ...
        if isinstance(self.min_samples_split, numbers.Integral):
            ...
            self._min_samples_split = max(2, self._min_samples_split)

        if isinstance(self.min_samples_leaf, numbers.Integral):
            if self.min_samples_leaf < 1:
                raise ValueError(
                    "min_samples_leaf must be an integer greater than 0 or "
                    "a float in (0.0, 1.0); got the integer {}".format(
            ...
            self._min_samples_leaf = max(1, self._min_samples_leaf)
